...
class AbundanceOsmosisRepository:
    ...
    def update_resource_location(
        ...
        self.conn.execute(
            ...
            WHERE id = ?
            """,
            (new_location, notes, datetime.utcnow().isoformat(), resource_id),
        )
        self.conn.commit()
    ...
    def get_ripe_knowledge_ripples(self, weeks_ago: int = 2) -> List[KnowledgeRipple]:
        ...
        """
        cutoff = (datetime.utcnow() - timedelta(weeks=weeks_ago)).isoformat()
        now = datetime.utcnow().isoformat()
        ...
        cursor = self.conn.execute(
            ...
            """,
            (cutoff, (datetime.utcnow() - timedelta(weeks=weeks_ago + 1)).isoformat()),
        )
    ...
    def mark_ripple_prompted(self, ripple_id: str) -> None:
        ...
        self.conn.execute(
            ...
            WHERE id = ?
            """,
            (datetime.utcnow().isoformat(), ripple_id),
        )
        self.conn.commit()
